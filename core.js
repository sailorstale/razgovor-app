...
// матрица клеток, четыре ряда в высоту и сколько угодно столбцов в ширину. У каждой
// карточки постоянное место «ряд:столбец». Настройка «картинок на экран» задаёт
// только окно: сколько рядов и столбцов видно. Окно двигается по матрице вправо на
...
// Корень лежит под ключом 'root', ключевые слова под 'core'. cells это объект
...
const MATRIX_ROWS = 4;   // высота матрицы задана словарём, не настройкой
const VOCAB_COLS  = 8;   // словарь нарисован под восемь столбцов, как у Avaz

// Двенадцать готовых размеров окна, как у Avaz: картинок на экран → ряды × столбцы.
const GRID_SIZES = [
  [1,1,1],[2,1,2],[3,1,3],[4,2,2],[8,2,4],[15,3,5],
  [24,4,6],[40,5,8],[60,6,10],[77,7,11],[96,8,12],[117,9,13],
].map(([n,rows,cols]) => ({ n, rows, cols }));
const GRID_DEFAULT = 15;

function gridSize(n) {
  return GRID_SIZES.find(g => g.n === n) || GRID_SIZES.find(g => g.n === GRID_DEFAULT);
}
// Старая настройка «столбцы × строки» переводится в ближайший готовый размер;
...
// четыре ряда внутри width столбцов в порядке чтения, а когда матрица заполнена,
...
// Сколько рядов окна размера n реально может быть заполнено: матрица выше четырёх
// не бывает, а в списке размеров есть окна на пять и более рядов.
...
// 5×8 дают две страницы (8 и 3 столбца), на окне 6×10 тоже две (10 и 1).
...
// Первая свободная клетка: сначала внутри восьми столбцов в порядке чтения, а если
// матрица заполнена, новый столбец справа. Новая карточка никого не сдвигает.
